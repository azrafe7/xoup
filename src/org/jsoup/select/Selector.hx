...
class Selector {
	...
	//NOTE(az): conflate with method below
    private function new(queryOrEvaluator:Dynamic, root:Element) {
        Validate.notNull(root);
        
		if (Std.is(queryOrEvaluator, String)) {
			var query:String = cast queryOrEvaluator;
			query = query.trim();
			Validate.notEmpty(query);

			this.evaluator = QueryParser.parse(query);
		} else if (Std.is(queryOrEvaluator, Evaluator)) {
			var evaluator:Evaluator = cast queryOrEvaluator;
			Validate.notNull(evaluator);
			
			this.evaluator = evaluator;
		} else {
			throw new IllegalArgumentException("Invalid value for queryOrEvaluator");
		}
     ...
     */
    //NOTE(az): conflate with below method
	public static function select(queryOrEvaluator:Dynamic, root:Element):Elements {
        return new Selector(queryOrEvaluator, root)._select();
    }
     ...
     * @return matching elements, empty if none
     */
	//NOTE(az): IdentityHashMap
    public static function selectIn(query:String, roots:Iterable<Element>):Elements {
        Validate.notEmpty(query);
        Validate.notNull(roots);
        var evaluator:Evaluator = QueryParser.parse(query);
        var elements = new ArrayList<Element>();
        var seenElements = new Map<Element, Bool>();
        // dedupe elements by identity, not equality

        for (root in roots) {
            var found:Elements = select(evaluator, root);
            for (el in found) {
                if (!seenElements.exists(el)) {
                    elements.add(el);
    ...
    private function _select():Elements {
